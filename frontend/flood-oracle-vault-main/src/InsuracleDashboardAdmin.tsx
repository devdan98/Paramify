...
export default function InsuracleDashboardAdmin({ setUserType }: ParamifyDashboardProps) {
  const [walletAddress] = useState('0x39f...2266');
  const [ethBalance, setEthBalance] = useState(9997.997);
  const [floodLevel, setFloodLevel] = useState(2000.0);
  const [newFloodLevel, setNewFloodLevel] = useState('');
  const [threshold] = useState(3000.0);
  const [coverageAmount, setCoverageAmount] = useState('');
  const [premium, setPremium] = useState(0);
  const [insuranceAmount, setInsuranceAmount] = useState(0);
  const [contractBalance, setContractBalance] = useState(2.1);
  ...
  const isAdmin = true;
  const isOracleUpdater = true;
  const isInsuranceAdmin = true;
  ...
  const handleBuyInsurance = () => {
    ...
    setTimeout(() => {
      ...
      const cost = calculatePremium(coverage);
      
      setEthBalance(prev => prev - cost);
      setInsuranceAmount(coverage);
      setContractBalance(prev => prev + cost);
  ...
  const roleStatuses = [
    { name: 'Admin', status: isAdmin },
    { name: 'Oracle Updater', status: isOracleUpdater },
    { name: 'Insurance Admin', status: isInsuranceAdmin }
  ];
