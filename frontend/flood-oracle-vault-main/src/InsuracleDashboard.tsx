...
export default function InsuracleDashboard({ setUserType }: InsuracleDashboardProps) {
  const [walletAddress] = useState('0x3c44cCf7036691B973D5A987B83c34fa093bc');
  const [ethBalance, setEthBalance] = useState(10000.0);
  const [floodLevel, setFloodLevel] = useState(0.0);
  const [newFloodLevel, setNewFloodLevel] = useState(0.0);
  const [threshold] = useState(3000.0);
  const [policyAmount, setPolicyAmount] = useState(10);
  const [premium] = useState(1.0);
  const [insuranceAmount, setInsuranceAmount] = useState(0.0);
  const [contractBalance, setContractBalance] = useState(5.5);
  ...
  const [activePolicyPremium, setActivePolicyPremium] = useState(0);
  const [activePolicyCoverage, setActivePolicyCoverage] = useState(0);
  ...
  const handleBuyInsurance = async () => {
    ...
    setTimeout(() => {
      ...
      setEthBalance(prevBalance => prevBalance - premium);
      setInsuranceAmount(policyAmount);
      setContractBalance(prevBalance => prevBalance + premium);
      setHasActivePolicy(true);
      setActivePolicyPremium(premium);
      setActivePolicyCoverage(policyAmount);
  ...
  const handleUpdateFloodLevel = () => {
    ...
    setTimeout(() => {
      ...
      if (newFloodLevel >= threshold && hasActivePolicy) {
        setTransactionStatus('Flood threshold exceeded! Insurance payout triggered.');
            ...
            <div className="space-y-4">
              ...
              <div className="bg-black/20 rounded-lg p-4">
                <div className="space-y-3">
                  <label className="block text-white font-medium">Update Flood Level</label>
                  <div className="flex space-x-3">
                    <input
                      type="number"
                      value={newFloodLevel}
                      onChange={(e) => setNewFloodLevel(Number(e.target.value))}
                      className="flex-1 bg-black/30 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      placeholder="Enter new flood level"
                    />
                    <button
                      onClick={handleUpdateFloodLevel}
                      disabled={isUpdatingFlood}
                      className="bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 disabled:from-gray-500 disabled:to-gray-600 text-white font-bold px-6 py-3 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-[1.02] disabled:transform-none"
                    >
                      {isUpdatingFlood ? (
              ...
              {hasActivePolicy ? (
                <div className="bg-green-500/20 border border-green-400/30 rounded-lg p-6">
                  ...
                  <div className="space-y-2">
                    <p className="text-white"><span className="text-green-300">Premium:</span> {activePolicyPremium.toFixed(1)} ETH</p>
                    <p className="text-white"><span className="text-green-300">Coverage:</span> {activePolicyCoverage.toFixed(1)} ETH</p>
